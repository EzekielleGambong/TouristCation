...
const listingsUploadController = require('../controllers/dogpetinformation.controller');
...
router.get('/recommend/:userId', async (req, res) => {
    try {
        ...
        const result = await fetch(`http://localhost:5000/recommend/dog/${userId}`)
        if (!result.ok) throw new Error('Waiting for your personalized recommendations.')
        const recommendedDogIds = await result.json()
        const parsedIds = recommendedDogIds.map(id => new mongoose.Types.ObjectId(id))
